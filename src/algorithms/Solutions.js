...
// we cant use number list thislarge in JS.
// Need to convert to strings and add each int manually
export const Solution13 = () => {
    ...
    //* Adds the running totla and the new number together
    const runAdd = () => {
        let finalInts = []
        let carry = 0
        ...
        //* Set the shorter num to have leading zeros to match the longer numbers length
        if(num2.length < num1.length){
            let lengthDiff = num1.length - num2.length
            let num2Leading0s = '0'.repeat(lengthDiff)
            num2 = num2Leading0s + num2
        }

        //* Add all the index values together one by one
        for(let i = num1.length-1; i >= 0; i--){
            let int1 = parseInt(num1[i])
            let int2 = parseInt(num2[i])
            let sum = carry + int1 + int2

            //* Set the 10 value that needs to be carried
            if(sum < 10){
                finalInts.unshift(sum)
                carry = 0
            } else {
                finalInts.unshift(sum-10)
                carry = 1
            }
        }

        if(carry === 1) finalInts.unshift(carry)
        const finalNum = finalInts.join('')
        runningTotal = finalNum
    }
    

    for(let i = nums.length-1; i >= 0; i--){ //* Get each number working backwards
        if(i === nums.length-1){ //* Set starting number
            runningTotal = nums[i]
        } else { //* Run addition calculations with the current number
            numToAdd = nums[i]
            runAdd()
        }
    }
